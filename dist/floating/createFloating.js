...
export function createFloating({ placement = "bottom", strategy = "absolute", middleware = [], platform, elements: { reference: externalReference, floating: externalFloating } = {}, transform = true, whileElementsMounted, open, } = {}) {
    let referenceNode = externalReference;
    let floatingNode = externalFloating;
    let cleanup = null;
    ...
    const floatingStyles = derived(_floatingStyles, ($styles) => Object.entries($styles).reduce((acc, [key, value]) => acc + `${key}:${value};`, ""));
    const data = writable({
        x: 0,
        y: 0,
        strategy,
